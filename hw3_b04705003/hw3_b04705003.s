...
#STEP4 integer operations
	...
	sw $v0, output
	move $s4, $v0
	j result # if is not operators above jump to return
T:
	# Create stack to save parameter
	addi $sp, $sp, -32
	# Store return address
	sw $ra , 4($sp)
	slti $t0, $a0, 2
	# if it n >= 2, go to recursive
	beq $t0, $zero, recursive
	# base case: assign $v0 to c
	move $v0, $s2
	addi $sp, $sp, 32
	jr $ra
recursive:
...
itoa:
	...
	# initial value: $t0, $t1, $t2
	addi $t0, $zero, 10 # set $t0 to 10
	addi $t1, $zero, 3 # output_ascii's index
	la $t2, output_ascii # load address of output_ascii
.convert:
	div $a0, $t0 # div $a0 by 10
	mfhi $t3 # remainder
	mflo $a0 # quotient
	addi $t3, $t3, '0' # $t3 = $t3 + '0'
	add $t4, $t1, $t2 # $t4 = output_ascii[$t1] address
	sb $t3, 0($t4) # store $t3 into output_ascii[$t1]
	addi $t1, $t1, -1 # $t1 -= 1
	bge $t1, $zero, .convert # if index is greater equal to 0, go to .convert iteratively
...
# ($s4 = fd_out)
	...
	la	$a1, output_ascii # a1 is buffer
	li	$a2, 4			# a2 is length to write
	syscall				# $v0 <= $s0 = fd
...
atoi:
    	...
    	lb      $t0, 0($a0)				# Load the first byte in argument 0
...
.num:
    	...
    	slti    $t2, $t0, 58        	# *str <= '9'
    	slti    $t3, $t0, '0'       	# *str < '0'
    	beq     $t2, $zero, .done		# if *str > '9', go to .done
    	bne     $t3, $zero, .done		# if *str < '0', go to .done
    	sll     $t2, $v0, 1				# t2 = num * 2
    	sll     $v0, $v0, 3				# num = num * 8
    	add     $v0, $v0, $t2       	# num *= 10, using: num = (num << 3) + (num << 1)
    	addi    $t0, $t0, -48			# offset ascii to number(str)
